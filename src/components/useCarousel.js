        ...
        <Carousel autoSlide={true} autoSlideInterval={10000}>
            {images.map((image, index) => (
                ...
                <div className="absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] w-[60%]">
                    <h2 className="opacity-0 z-20 text-[36px] group-[.slide-active]:opacity-100 transition-all duration-[2000ms]">{imagesTitle[index]}</h2>
                    <p className="opacity-0 z-20 text-[24px] group-[.slide-active]:opacity-100 transition-all duration-[3000ms]">{imageText[index]}</p>
                </div>
                </div>
            ))}
        </Carousel>
